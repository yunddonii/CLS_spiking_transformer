...
class SpikLinearLayer(nn.Module):
    def __init__(self, in_dim, out_dim=None, tau=2.0, lif_bias=False):
        ...
        self.lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., lif_bias=False, tau=2.0):
        ...
        self.fc1_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())

        self.fc2_linear = nn.Linear(hidden_features, out_features, bias=lif_bias)
        self.fc2_bn = nn.BatchNorm1d(out_features)
        self.fc2_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
...
class SSA_rel_scl(nn.Module):
    def __init__(self, dim, seq_len, num_heads=8, pe=True, lif_bias=False, tau=2.0, attn='MSSA') -> None:
        ...
        if attn == 'SSA' :  self.q_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())

        self.k_linear = nn.Linear(dim, dim, bias=lif_bias)
        self.k_bn = nn.BatchNorm1d(dim)
        if attn == 'SSA' : self.k_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
        ...
        self.v_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
        ...
        self.proj_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
        ...
        # self.rel_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy')
...
class MutualCrossAttention(nn.Module):
    def __init__(self, dim, out_seq=True, lif_bias=False, num_heads=8, tau=2.0, attn='MSSA') -> None:
        ...
        # self.q_lif1 = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
        self.q_linear2 = nn.Linear(dim, dim, bias=lif_bias)
        self.q_bn2 = nn.BatchNorm1d(dim)
        if attn == 'SSA' : self.q_lif2 = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
...
class MemoryUpdate(nn.Module):
    def __init__(self, seq_len, dim, out_seq=True, lif_bias=False, num_heads=8, tau=2.0) -> None:
        ...
        self.gate_lif = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
...
class TokenMixer(nn.Module):
    def __init__(self, seq_len, tau=2.0, bias=False):
        ...
        self.lif1 = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
        
        # self.linear2 = nn.Linear(seq_len, seq_len, bias=bias)
        # self.bn2 = nn.BatchNorm1d(seq_len)
        # self.lif2 = MultiStepLIFNode(tau=tau, detach_reset=True, backend='cupy', surrogate_function=surrogate.ATan())
