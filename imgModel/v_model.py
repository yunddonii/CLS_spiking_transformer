...
from v_layers import SpkEncoder
from v_encoder import Split2Patch, Embedding, Block, TemporalBlock
from v_layers import SpkEncoder, AttentionPool1d, MutualCrossAttention, Consolidation, SSA_rel_scl, TokenMixer, MemoryUpdate
...
class Decoder(nn.Module):
    def __init__(self, embed_dim, patch_size=[16, 32], ratio=8, tau=2.0, bias=True) -> None:
        ...
        # self.reconstruction = nn.Linear(embed_dim, original_size, bias=bias)
        
    def forward(self, x, out_rec=True):
        """
        [original] x: B x H x W x C (embed_dim)
        [MyModel] x: T x B x 1 x D
...
class Spikformer(nn.Module):
    def __init__(self, gating=['original', 'ablation'], 
                 ...
                 img_h=32, img_w=32, patch_h=16, patch_w=16,
                num_classes=2, 
                time_num_layers=2,
        ...
        self.split_data = Split2Patch((img_h, img_w), (patch_h, patch_w))
        ...
        self.time_block = TemporalBlock(num_layers=time_num_layers,
                                        embed_dim=embed_dim,
                                        lif_bias=lif_bias,
                                        num_heads=num_heads,
                                        tau=tau,
        ...
        if self.train_mode == 'training' : 
            self.weak_decoder = Decoder(embed_dim=embed_dim, patch_size=patch_dim, ratio=8, tau=tau, bias=lif_bias)
